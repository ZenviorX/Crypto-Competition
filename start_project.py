...
def process_cwd(pid: int) -> str:
    """
    Return the working directory of a POSIX process.

    npm may spawn Vite as a child whose command line no longer
    contains the AgentGuard repository path. In that case cwd
...
def related(pid: int, service: str) -> bool:
    ...
    # npm --prefix may spawn a Vite child whose command
    # line no longer contains ROOT. Recognize that child
    # through its working directory as well.
