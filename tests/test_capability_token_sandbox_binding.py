...
def _request(sandbox_profile: str, capability_token: str = "") -> ToolProxyAuthorizeRequest:
...
def test_capability_token_cannot_be_reused_with_different_sandbox_profile():
    reset_token_ledger()

    phase1 = prepare_tool_authorization(
        _request(sandbox_profile="local_readonly")
    )
    ...
    assert "different sandbox profile" in reasons
