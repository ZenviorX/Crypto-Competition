    ...
    <div className="page-grid evidence-page">
      <section className="evidence-hero">
        <div>
          ...
          <h1>运行证据与审计时间线</h1>
          <p>
            本页只展示后端真实审计日志、Hybrid Sandbox evidence 和最新测试摘要；不会用浏览器 Mock 数据替代失败请求。
          </p>
        </div>
        <div className="evidence-trust-card">
          <span>Evidence Source</span>
          <strong>{overview?.source ?? 'local_runtime_files'}</strong>
          <small>Audit Log + Native/Docker Sandbox + Test Results</small>
        </div>
      </section>

      <div className="metric-grid evidence-metrics">
        <MetricCard title="本地运行记录" value={compactNumber(overview?.totalRequests ?? 0)} hint="审计日志与沙箱 evidence 聚合" icon="dashboard" />
        <MetricCard title="阻断请求" value={compactNumber(overview?.blockedRequests ?? blocked.length)} hint="decision=deny 的真实记录" icon="shield" />
        <MetricCard title="待确认请求" value={compactNumber(overview?.confirmRequests ?? pending.length)} hint="需要审批的副作用操作" icon="spark" />
        <MetricCard title="沙箱证据" value={compactNumber(evidenceRuns)} hint="Native / Docker evidence.json" icon="check" />
      </div>

      <Section
        eyebrow="Authorization Records"
        title="最近授权与沙箱记录"
        description="表格随实时数据层自动更新；审批操作不再使用不存在的前端伪接口。"
        actions={<Badge tone={pending.length ? 'yellow' : 'green'}>{pending.length} 个待确认</Badge>}
      >
        <RequestTable rows={requests.slice(0, 12)} />
      </Section>
      ...
      <section className="evidence-grid">
        <Section
          ...
          title="最新审计事件"
          description="授权、阻断和沙箱执行会按时间进入本地审计记录。"
        >
        ...
        <Section
          ...
          title="证据链说明"
          description="展示项目真正的可追责价值，而不是普通日志列表。"
        >
