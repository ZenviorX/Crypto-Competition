...
import {
  ...
  type ResearchReport
} from './pages/ResearchComparisonPage';
...
export default function App() {
      ...
      case 'research':
        return {
          ...
          items: [
            {
              label: '对比策略',
              value: `${researchReport ? 3 : 0} 种`,
              tone: researchReport ? 'blue' : 'gray'
            },
            {
              label: '对比样例',
              value: `${researchReport?.total_cases ?? 0} 条`,
              tone: researchReport ? 'purple' : 'gray'
            },
            {
              label: '结果状态',
              value: researchReport ? '已生成' : '等待生成',
              tone: researchReport ? 'green' : 'gray'
            }
          ]
        };
    }
  ...
  const content: Record<PageKey, JSX.Element> = {
    ...
    research: (
      <ResearchComparisonPage
        onReportChange={setResearchReport}
      />
    )
  };
